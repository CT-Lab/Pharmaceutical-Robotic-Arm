...
class DQNAgent(object):
  ...
  def __init__(self,
    ...
    # self.total_num_action = math.pow(self.num_actions, len(self.target_name))
    ...
    with tf.device(tf_device):
      ...
      obs_shape = (1,) + self.observation_shape + (stack_size,)
  ...
  def _network_template_actor(self, obs):
    ...
    return tf.clip_by_value(a, -1, 1, name='scaled_a')
    # """
    # net = tf.cast(obs, tf.float32)
    # # net = tf.div(net, 255.)
  ...
  def _build_networks(self):
    ...
    self._replay_net_outputs_C = self.online_convnet_C(self._replay.states, self._replay_net_outputs_A)
  ...
  def _build_train_op(self):
    ...
    td_error = tf.losses.mean_squared_error(labels=q_target, predictions=self._replay_net_outputs_C)
    a_loss = - tf.reduce_mean(self._replay_net_outputs_C)
    if self.summary_writer is not None:
  ...
  def _build_sync_op(self):
    ...
    sync_qt_ops = [[tf.assign(ta, ea, use_locking=True), tf.assign(tc, ec, use_locking=True)]
                   for ta, ea, tc, ec in zip(self.at_params, self.ae_params, self.ct_params, self.ce_params)]

    # sync_qt_ops = [[tf.assign(ta, (1-self.tau)*ta + self.tau*ea, use_locking=True), tf.assign(tc, (1-self.tau)*tc+self.tau*ec, use_locking=True)]
    #                for ta, ea, tc, ec in zip(self.at_params, self.ae_params, self.ct_params, self.ce_params)]
  ...
  def step(self, reward, observation, state):
    ...
    # print("_observation shape", self._observation.shape)
  ...
  def _record_obs_state(self, observation, state):
    ...
    self.obs[0, ..., -1] = self._observation
    self._state = state
    self.state[0,...] = state
