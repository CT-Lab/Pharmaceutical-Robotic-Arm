...
from agents.ddpg.evaluate import Evaluate, Model
# import torch
...
# NATURE_DQN_OBSERVATION_SHAPE = (100, 100)     # for gray
NATURE_DQN_OBSERVATION_SHAPE = (100, 100, 3)  # for rgb
...
class DDPGAgent(object):
  ...
  def _import_BC(self):
    print('build behavior cloning')
    self.bc_model = Evaluate()
    self.bc_model.load_model('agents/ddpg/data/models/', 'model-52-400.pt')
    self.bc_model.load_unnorm('agents/ddpg/data/df_min.pickle', 'agents/ddpg/data/df_max.pickle')
    self.bc_model.set_command('grasp 8cm bottle')    
  ...
  def _network_template_actor(self, obs):
    ...
    # net = tf.layers.dense(net, 512, activation=tf.nn.relu)
    # a = tf.layers.dense(net, self.num_actions, activation=tf.nn.tanh)

    return tf.clip_by_value(a, self.action_low, self.action_high, name='scaled_a')
  ...
  def _network_template_critic(self, state, action):
    ...
    # net = tf.layers.dense(net, 512, activation=tf.nn.relu)
    # net = tf.layers.dense(net, 512, activation=tf.nn.relu)
    # q = tf.layers.dense(net, 1, activation=tf.nn.relu)
  ...
  def _build_networks(self):
    ...
    self._net_outputs_A = self.online_convnet_A(self.obs_ph)
    # self._net_outputs_A = self.online_convnet_A(self.state_ph)
    # print(self._net_outputs_A)

    self._replay_net_outputs_A = self.online_convnet_A(self._replay.observations)
    self._replay_next_target_net_outputs_A = self.target_convnet_A(self._replay.next_observations)
  ...
  def begin_episode(self, observation, state):
    ...
    if not self.eval_mode:
      self._train_step()
      # self.action = self._behavior_action(state)
      self.action = self._select_train_action(state)
    else:
      self.action = self._select_eval_action()
  ...
  def step(self, reward, observation, state):
    ...
    self._record_obs_state(observation, state)
    # cv2.imwrite('temp.jpg', self._observation)
  ...
  def _select_train_action(self, state):
    ...
    if epsilon > 0.2:
      ...
      print(epsilon)
      action = self.bc_model.predict(state[:5], state[-7:], [0,1.57])
    # action = self._sess.run(self._net_outputs_A, {self.obs_ph: self.obs})[0]
    action = action.clip(self.action_low, self.action_high)
    
    return action

  def _select_eval_action(self):
  ...
  def _behavior_action(self, state):
    # a = self._select_train_action()
    if np.random.random() > 0.2:
      # a += np.random.normal(a, 0.2*0.05)
      a = self._select_train_action(state)
      # a += np.random.normal(a, 0.01)
      # a = a.clip(self.action_low, self.action_high)
    else:
      a = np.random.uniform(self.action_low, self.action_high, self.num_actions)
    return a
  ...
  def _train_step(self):
    ...
    # have been run. This matches the Nature DQN behavior.
  ...
  def _record_obs_state(self, observation, state):
    ...
    self._state = state[:12]
    self.state[0,...] = state[:12]
