...
# NATURE_DQN_OBSERVATION_SHAPE = (84, 84)     # for gray
...
class DQNAgent(object):
  ...
  def _network_template_actor(self, obs):
    ...
    return tf.clip_by_value(a, -0.05, 0.05, name='scaled_a')
  ...
  def _behaviour_action(self):
    a = self._select_action()
    if random.random() > 0.2:
      a += np.random.normal(0, 0.1*0.05, len(a))
      a = a.clip(-0.05, 0.05)
    return a
