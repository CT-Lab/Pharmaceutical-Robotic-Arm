...
class OutOfGraphReplayBuffer(object):
  ...
  def __init__(self,
    ...
    tf.logging.info('\t state_shape: %s', str(state_shape))
    tf.logging.info('\t state_dtype: %s', str(state_dtype))
    ...
    # self._obs_shape = self._observation_shape + (self._stack_size,) # for gray
    self._obs_shape = self._observation_shape                         # for rgb
    ...
    print('get_storage_signature ', self.get_storage_signature())
  ...
  def get_storage_signature(self):
    ...
    storage_elements = [
        ...
        ReplayElement('state', self._state_shape, self._state_dtype),
        ReplayElement('action', self.action_dim, np.float32),
  ...
  def add(self, observation, state, action, reward, terminal, *args):
    ...
    Args:
      ...
      *args: extra contents with shapes and dtypes according to
        extra_storage_types.
    """
    self._check_add_types(observation, state, action, reward, terminal, *args)
    ...
    self._add(observation, state, action, reward, terminal, *args)
  ...
  def get_observation_stack(self, index):
    obs = self.get_range(self._store['observation'],
                           index - self._stack_size + 1, index + 1)
    # The stacking axis is 0 but the agent expects as the last axis.
    # return np.moveaxis(obs, 0, -1)  # origin
    return obs

  def get_state_stack(self, index):
    return self.get_range(self._store['state'],
                          index - self._stack_size + 1, index + 1)
    ...
    for batch_element, state_index in enumerate(indices):
      ...
      for element_array, element in zip(batch_arrays, transition_elements):
        if element.name == 'observation':
          ...
          element_array[batch_element] = self.get_state_stack((state_index))
  ...
  def get_transition_elements(self, batch_size=None):
    ...
    transition_elements = [
        ReplayElement('observation', (batch_size,) + self._obs_shape,
                      self._observation_dtype),
        ReplayElement('state', (batch_size, ) + self._state_shape,
                      self._state_dtype),
        ReplayElement('action', (batch_size,) + self.action_dim, np.float32),
        ReplayElement('reward', (batch_size, 1), np.float32),
        ReplayElement('next_observation', (batch_size,) + self._obs_shape,
                    self._observation_dtype),
        ReplayElement('next_state', (batch_size,) + self._state_shape,
                      self._state_dtype),
        ReplayElement('terminal', (batch_size, 1), np.uint8),
...
class WrappedReplayBuffer(object):
  ...
  def __init__(self,
    ...
    if wrapped_memory is not None:
      ...
      self.memory = OutOfGraphReplayBuffer(
          observation_shape, state_shape, action_dim, stack_size, replay_capacity, batch_size,
  ...
  def add(self, observation, state, action, reward, terminal, *args):
    ...
    Args:
      ...
      *args: extra contents with shapes and dtypes according to
        extra_storage_types.
    """
    self.memory.add(observation, state, action, reward, terminal, *args)
  ...
  def unpack_transition(self, transition_tensors, transition_type):
    ...
    self.next_observations = self.transition['next_observation']
